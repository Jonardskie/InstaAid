...
export async function POST(req: Request) {
  try {
    ...
    // 2. Send professional deletion notification email via Gmail SMTP
    ...
    if (smtpEmail && smtpPassword && recipientEmail) {
      try {
        ...
        await transporter.sendMail({
          ...
          to: recipientEmail,
          subject: "Your InstaAid Account Has Been Deleted",
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 560px; margin: auto; background: #f8fafc; padding: 32px 20px; border-radius: 16px; border: 1px solid #e2e8f0;">
              ...
              <div style="background: #ffffff; padding: 28px 24px; border-radius: 12px; border: 1px solid #e2e8f0;">
                ...
                <p style="font-size: 14px; color: #475569; line-height: 1.6;">
                  We are writing to inform you that your InstaAid account has been <strong>deleted</strong> by an administrator.
                </p>
                <p style="font-size: 14px; color: #475569; line-height: 1.6;">
        ...
        console.log(`✅ [InstaAid] Deletion notification email sent to: ${recipientEmail}`)
      } catch (mailError) {
        console.warn("⚠️ Deletion notification email failed:", mailError)
      }
    }
